#!/usr/bin/env python3
"""
Fix API rules by deleting broken ones and recreating with proper IP addresses
"""
...
logger = logging.getLogger('FixAPIRules')
...
class APIRuleFixer:
    ...
    def create_rule(self, rule_config: Dict) -> bool:
        """Create a firewall rule with proper IP addresses"""
        try:
    ...
    def get_correct_rules(self) -> List[Dict]:
        """Generate correct firewall rules with proper IP addresses"""
        rules = []
        rule_index = 20001
    ...
    def fix_rules(self) -> Dict:
        """Delete broken rules and create correct ones"""
        results = {
            'deleted_rules': 0,
            'created_rules': 0,
            'failed_deletions': 0,
        ...
        # Delete broken rules (those with null src_ip/dst_ip)
        logger.info("Deleting broken rules...")
        for rule in existing_rules:
            if rule.get('name', '').startswith('Block_') and (rule.get('src_ip') is None and rule.get('dst_ip') is None):
        ...
        logger.info("Creating correct rules with proper IP addresses...")
    ...
    def verify_rules(self) -> Dict:
        ...
        try:
            existing_rules = self.get_existing_rules()
            working_rules = [r for r in existing_rules if r.get('src_ip') or r.get('dst_ip')]
            broken_rules = [r for r in existing_rules if r.get('name', '').startswith('Block_') and not (r.get('src_ip') or r.get('dst_ip'))]
...
def main():
    print("=" * 80)
    print("FIXING API FIREWALL RULES")
    print("=" * 80)
    
    api_key = "C5tSpDSC2J_bZy9oqYwBDesWoUP1sHIK"
    fixer = APIRuleFixer(api_key=api_key)
    
    print("\nFixing broken API rules...")
    results = fixer.fix_rules()
    ...
    print("\nVerifying rules...")
    verification = fixer.verify_rules()
    ...
    if verification.get('status') == 'HEALTHY':
        ...
        print(f"✅ Rules have proper IP addresses")
