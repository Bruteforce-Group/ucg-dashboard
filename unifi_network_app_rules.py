#!/usr/bin/env python3
"""
UniFi Network Application Firewall Rules Creator
Creates proper UniFi Network Application firewall rules using the correct API structure
Based on the actual UniFi Network Application rule format
"""
...
logger = logging.getLogger('UniFiNetworkAppRules')

class UniFiNetworkAppRuleManager:
    """Manage UniFi Network Application firewall rules via API"""
    ...
    def authenticate_with_api_key(self, api_key: str) -> bool:
        ...
        try:
            ...
            # Test authentication by getting meta info
            response = self.session.get(
                f"{self.controller_url}/proxy/protect/integration/v1/meta/info",
                timeout=10
            )
    ...
    def get_existing_firewall_rules(self) -> List[Dict]:
        """Get existing firewall rules from UniFi Network Application"""
        try:
            # Try the standard UniFi API endpoint for firewall rules
            response = self.session.get(
                f"{self.controller_url}/api/s/{self.site_id}/rest/firewallrule",
                timeout=10
            )
    ...
    def create_firewall_rule(self, rule_config: Dict) -> bool:
        """Create a firewall rule in UniFi Network Application"""
        try:
            # Use the standard UniFi API endpoint for firewall rules
    ...
    def create_unmanaged_range_blocking_rules(self) -> Dict:
        ...
        logger.info("Creating UniFi Network Application firewall rules to block unmanaged ranges...")
        
        # Define all the rules we need to create based on UniFi Network Application format
    ...
    def _get_unmanaged_blocking_rules(self) -> List[Dict]:
        """Get firewall rules to block unmanaged ranges in UniFi Network Application format"""
        rules = []
        
        # IPv4 Unmanaged Ranges - Block incoming from unmanaged ranges
        ipv4_ranges = [
        ...
        # IPv6 Unmanaged Ranges - Block incoming from unmanaged ranges
        ipv6_ranges = [
            ("fc00::/7", "IPv6_ULA"),
...
def main():
    """Main function to create UniFi Network Application firewall rules"""
    print("=" * 80)
    print("UNIFI NETWORK APPLICATION FIREWALL RULES CREATION")
    print("=" * 80)
    ...
    # Initialize manager
    manager = UniFiNetworkAppRuleManager()
    
    # Authenticate with API key
    print("\nAuthenticating with UCG-Fiber controller using API key...")
    if not manager.authenticate_with_api_key(api_key):
        print("❌ API key authentication failed!")
        print("This might be because:")
        print("1. The API key is invalid or expired")
        print("2. UCG-Fiber uses a different API structure")
    ...
    if verification.get('status') == 'healthy':
        print(f"\n🎉 SUCCESS! All unmanaged IPv4 and IPv6 ranges are now blocked via UniFi Network Application!")
        ...
        print(f"✅ View rules at: https://192.168.22.1:8443")
    else:
